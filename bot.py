...
def env_role(name: str) -> int | None:
    ...
    if not value or value.upper() in {"ID", "ID_РОЛИ", "NONE"}:
        print(f"❌ Не указана переменная роли: {name}")
        return None
...
async def send_embed(channel_id: int, text: str, color: int = 0x2B2D31):
...
async def handle_new_registry(row: dict, guild: discord.Guild, registry_rows: list[dict]):
    ...
    text = (
        ...
        f"2. 🆕 Принят на работу на ранг {rank_tag} // {reason if reason else 'Собеседование'}\n"
        f"3. 🗓️ Дата принятия: **{date_now()}**"
    )

    await send_embed(HR_CHANNEL_ID, text, 0x5865F2)
...
async def handle_fire(row: dict, guild: discord.Guild, registry_rows: list[dict]):
    ...
    text = (
        ...
        f"3. ⭕ Уволен по причине: {reason if reason else 'Не указана'}\n"
        f"4. 🗓️ Дата увольнения: **{date_now()}**"
    )

    await send_embed(HR_CHANNEL_ID, text, 0xED4245)
...
async def handle_department_transfer(row: dict, guild: discord.Guild, registry_rows: list[dict]):
    ...
    text = (
        f"1. {user_line(auditor, auditor_static, auditor_name)}\n"
        f"2. {user_line(target, target_static, target_name)}\n"
        f"3. 🔁 Переведен в отдел: {to_tag} // {reason if reason else 'Не указана'}\n"
...
async def handle_punishment(row: dict, guild: discord.Guild, registry_rows: list[dict]):
    ...
    await send_embed(PUNISH_CHANNEL_ID, text, 0xED4245)
    ...
    if next_warn >= 3:
        fire_text = (
            f"1. {user_line(auditor, auditor_static, auditor_name)}\n"
            f"2. {user_line(target, target_static, target_name)}\n"
            f"3. ⭕ Уволен по причине: 3/3 взысканий\n"
            f"4. 🗓️ Дата увольнения: **{date_now()}**"
        )

        await send_embed(HR_CHANNEL_ID, fire_text, 0xED4245)
        await fire_member(target, guild)
